...
object LockMaster {
  ...
  private val lockPath = LocalStorageService.constructLocalContentPath(AssetCategory.META, "lock.json")

  fun acquireLock(id: String): Boolean = when {
    Files.notExists(lockPath) -> lockProcess(id)
    canBreakLock() -> breakAndLockProcess(id)
    else -> false
  }

  private fun canBreakLock(): Boolean = readLock()
    .map {
      Duration.between(
        it.lockedDate,
        Instant.now()
  ...
  private fun breakAndLockProcess(id: String): Boolean = when {
    breakLock() -> lockProcess(id)
    else -> false
  }
  ...
  private fun holdingLock(id: String): Boolean = readLock()
    .map { it.lockedBy == id }
    .orElse(false)

  private fun breakLock(): Boolean = when {
    Files.exists(lockPath) -> runSafelyWithResult({ Files.delete(lockPath); true }) {
  ...
  private fun lockProcess(id: String): Boolean = writeLock(Lock(id, Instant.now()))

  private fun readLock(): Optional<Lock> = runSafelyWithResult({
  ...
  private fun writeLock(lock: Lock): Boolean = runSafelyWithResult({
